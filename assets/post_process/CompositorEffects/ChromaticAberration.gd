...
@export_range(0.0, 10.0, 0.01) var aberration:float=1.0
...
func initialize_shader() -> void:
	...
	var shader_file : RDShaderFile = load("uid://bqet1wnfmf40v")
	if not shader_file:
		print("❌ Шейдер не найден по пути: res://Shaders/ComputeShader/ChromaticAberration.glsl")
		return
...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	...
	for view in scene_buffers.get_view_count():
		...
		push_constants.append(aberration)
