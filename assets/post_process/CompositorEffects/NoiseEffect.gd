...
class_name NoiseEffect

@export_range(0.0, 10.0, 0.01) var noise_strength:float=1.0
...
func initialize_shader() -> void:
	...
	var shader_file : RDShaderFile = load("uid://csucvvn7n1g4u")
	if not shader_file:
		print("❌ Шейдер не найден по пути: res://Shaders/ComputeShader/NoiseEffect.glsl")
		return
...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	...
	for view in scene_buffers.get_view_count():
		...
		push_constants.append(noise_strength)
		push_constants.append(float(Time.get_ticks_msec()) * 0.001)
