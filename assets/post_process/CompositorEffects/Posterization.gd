...
class_name Posterization

@export_range(0, 256, 2, "prefer_slider") var levels: float = 128
...
func initialize_shader() -> void:
	...
	var shader_file : RDShaderFile = load("uid://cdd317un2blbj")
	if not shader_file:
		print("❌ Шейдер не найден по пути: res://Shaders/ComputeShader/Posterization.glsl")
		return
