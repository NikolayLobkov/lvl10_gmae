...
class_name Stippling

@export var dot_size : float = 0.8
@export var dot_density:float = 15.0
...
func initialize_shader() -> void:
	...
	var shader_file : RDShaderFile = load("uid://xyti6ngawkth")
	if not shader_file:
		print("❌ Шейдер не найден по пути: res://Shaders/ComputeShader/Stippling .glsl")
		return
...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	...
	for view in scene_buffers.get_view_count():
		...
		push_constants.append(dot_size)
		push_constants.append(dot_density)
